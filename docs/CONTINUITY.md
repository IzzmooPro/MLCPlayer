...
- Aynı uncommitted UX ağacındaki görsel bütünlük paketi ürünün turuncu vurgu, hover/pressed renkleri ve Segoe UI Variable Text→Segoe UI font zincirini `app.config` altında tek kaynağa bağladı; eski mavi seçim/slider vurguları ve ikinci turuncu kaldırıldı. Başlık, playlist, başlangıç, Medya Bilgisi, kısa bilgi, overlay, altyazı ve güncelleyici yüzeyleri ortak kimliği kullanıyor; klavye focus durumları nötr ve görünür, daha önce reddedilen ikincil Klasör düğmesinde turuncu focus geri getirilmedi. Beş kırmızı sözleşme sonrası ana yüzey **192 passed**, ikinci etki halkası **561 passed**, kalite/çeviri **51 passed** ve tam deterministik paket **5210 passed / 11 skipped** verdi (`EV-20260830-004`). Sonraki dar düzeltmede başlangıç yüzeyinin her 100 ms'de tekrarlanan `raise_()` z-order/input churn'i visibility geçişine indirildi; Kısayollar penceresi 450×520'ye sıkıştırılıp `Tamam` düğmesi `accept()` ile gerçek Accepted kapanışa bağlandı. Etkilenen aileler **53 + 236 passed**, continuity **10 passed** verdi (`EV-20260830-005`). Taze Windows insan görsel kabulü ve bağımsız karşıt agent GO'su henüz yoktur.
- Aynı uncommitted UX ağacındaki görsel bütünlük paketi ürünün turuncu vurgu, hover/pressed renkleri ve Segoe UI Variable Text→Segoe UI font zincirini `app.config` altında tek kaynağa bağladı; eski mavi seçim/slider vurguları ve ikinci turuncu kaldırıldı. Başlık, playlist, başlangıç, Medya Bilgisi, kısa bilgi, overlay, altyazı ve güncelleyici yüzeyleri ortak kimliği kullanıyor; klavye focus durumları nötr ve görünür, daha önce reddedilen ikincil Klasör düğmesinde turuncu focus geri getirilmedi. Beş kırmızı sözleşme sonrası ana yüzey **192 passed**, ikinci etki halkası **561 passed**, kalite/çeviri **51 passed** ve tam deterministik paket **5210 passed / 11 skipped** verdi (`EV-20260830-004`). Sonraki dar düzeltmede başlangıç yüzeyinin her 100 ms'de tekrarlanan `raise_()` z-order/input churn'i visibility geçişine indirildi; Kısayollar penceresi 450×520'ye sıkıştırılıp `Tamam` düğmesi `accept()` ile gerçek Accepted kapanışa bağlandı. Başarılı dosya/klasör açılışından sonra focus video yüzeyine taşındı; böylece açıcı ikonunun oynatma sırasında seçili kalması giderildi. Etkilenen aileler **53 + 236 + 139 passed**, continuity **10 passed** verdi (`EV-20260830-005`–`006`). Taze Windows insan görsel kabulü ve bağımsız karşıt agent GO'su henüz yoktur.
- Aynı uncommitted UX ağacındaki görsel bütünlük paketi ürünün turuncu vurgu, hover/pressed renkleri ve Segoe UI Variable Text→Segoe UI font zincirini `app.config` altında tek kaynağa bağladı; eski mavi seçim/slider vurguları ve ikinci turuncu kaldırıldı. Başlık, playlist, başlangıç, Medya Bilgisi, kısa bilgi, overlay, altyazı ve güncelleyici yüzeyleri ortak kimliği kullanıyor; klavye focus durumları nötr ve görünür, daha önce reddedilen ikincil Klasör düğmesinde turuncu focus geri getirilmedi. Beş kırmızı sözleşme sonrası ana yüzey **192 passed**, ikinci etki halkası **561 passed**, kalite/çeviri **51 passed** ve tam deterministik paket **5210 passed / 11 skipped** verdi (`EV-20260830-004`). Sonraki dar düzeltmede başlangıç yüzeyinin her 100 ms'de tekrarlanan `raise_()` z-order/input churn'i visibility geçişine indirildi; Kısayollar penceresi 450×520'ye sıkıştırılıp `Tamam` düğmesi `accept()` ile gerçek Accepted kapanışa bağlandı. Başarılı dosya/klasör açılışından sonra focus video yüzeyine taşındı; böylece açıcı ikonunun oynatma sırasında seçili kalması giderildi. Tek komutluk ana pencere kabul runner'ı eklendi: 13 fiziksel grubu kanonik sırada varsayılan 3 temiz döngüyle çalıştırıyor, ilk başarısızlıkta fail-closed duruyor ve tek JSON özeti üretiyor. Etkilenen aileler **53 + 236 + 139 + 141 passed**, continuity **10 passed** verdi (`EV-20260830-005`–`007`). Taze Windows insan görsel kabulü ve bağımsız karşıt agent GO'su henüz yoktur.
- Aynı uncommitted UX ağacındaki görsel bütünlük paketi ürünün turuncu vurgu, hover/pressed renkleri ve Segoe UI Variable Text→Segoe UI font zincirini `app.config` altında tek kaynağa bağladı; eski mavi seçim/slider vurguları ve ikinci turuncu kaldırıldı. Başlık, playlist, başlangıç, Medya Bilgisi, kısa bilgi, overlay, altyazı ve güncelleyici yüzeyleri ortak kimliği kullanıyor; klavye focus durumları nötr ve görünür, daha önce reddedilen ikincil Klasör düğmesinde turuncu focus geri getirilmedi. Beş kırmızı sözleşme sonrası ana yüzey **192 passed**, ikinci etki halkası **561 passed**, kalite/çeviri **51 passed** ve tam deterministik paket **5210 passed / 11 skipped** verdi (`EV-20260830-004`). Sonraki dar düzeltmede başlangıç yüzeyinin her 100 ms'de tekrarlanan `raise_()` z-order/input churn'i visibility geçişine indirildi; Kısayollar penceresi 450×520'ye sıkıştırılıp `Tamam` düğmesi `accept()` ile gerçek Accepted kapanışa bağlandı. Başarılı dosya/klasör açılışından sonra focus video yüzeyine taşındı; böylece açıcı ikonunun oynatma sırasında seçili kalması giderildi. Tek komutluk ana pencere kabul runner'ı eklendi: 13 fiziksel grubu kanonik sırada varsayılan 3 temiz döngüyle çalıştırıyor, ilk başarısızlıkta fail-closed duruyor ve tek JSON özeti üretiyor. I: sürücüsündeki iki video ile ilk gerçek ölçümde buttons **21/21**, window_resize **12/12** geçti; birleşik döngü playlist açıkken `overlayFullscreen` için `entered=False` ile başarısız oldu (`EV-20260830-005`–`008`). Taze Windows insan görsel kabulü tamamlanmadı; bağımsız karşıt agent GO'su tek-agent kısıtı nedeniyle yoktur.
