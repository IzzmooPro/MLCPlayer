# MLC Player continuity tarihçesi

> **TARİHSEL ARŞİV:** Bu dosya, şişen eski `CONTINUITY.md` kronolojisinin veri
> kaybı olmadan ayrılmış halidir. Güncel durum veya sıradaki iş buradan
> çıkarılmaz; yalnız `docs/CONTINUITY.md` kullanılır. Makinece kanıtın kanonik
> kaynağı `docs/VERIFICATION_LEDGER.json` olarak kalır.
...
173. EV-013 kayıt paketi PR #47'nin zorunlu hosted belge kapısından **247
   passed** ve `LEDGER_APPEND_ONLY_OK` ile geçti; ayrı onaylı iki ebeveynli
   `f4255f5` merge commit'i üzerinden master'a alındı. Ardından kullanıcının
   kurulum görsel planı onayıyla sabit UX kararları
   `docs/INSTALLER_UX_PLAN.md` içine yazıldı ve mevcut MLC marka varlığına
   dayanan üç karşılaştırılabilir bitmap yönü üretildi (`EV-20260825-014`).
   Görseller private visualization alanındadır ve Git'e eklenmedi. Bunlar
   tasarım önizlemesidir; gerçek Inno Setup çıktısı, installer davranışı,
   build veya kurulum kanıtı değildir.

## Tarihsel sıradaki adım snapshot'ı

Üç installer önizlemesini kullanıcıya göster ve açık A, B veya C görsel yön
seçimini al. Seçimden önce installer kodu, build veya kurulum yapma.

## Tarihsel sonraki sıra snapshot'ı

1. Private görsel/native artifact'ları hiçbir Git işlemine ekleme.
2. Kullanıcının seçtiği installer yönünün metin, odak ve ekran akışını
   kesinleştir; uygulama değişikliği için ayrıca açık onay al.
3. Installer uygulamasını ayrı `codex/installer-experience` dalında
   regresyon-first yürüt; build ve fiziksel kurulum onaylarını ayır.
