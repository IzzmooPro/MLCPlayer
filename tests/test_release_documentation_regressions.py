...
def test_addon_process_gate_matches_measured_ytdlp_and_deno_behavior():
    ...
    for phrase in (
            "gercek urun url'si", "player tarafindan baslatilan",
            "exact kurulu yt-dlp", "pid/ebeveyn/komut korelasyonu",
            "position/duration", "basarili oynatma",
            "deno yalniz gercek cikarim gerektirirse",
            "deno yoklugu basarisizlik degildir", "bagimsiz",
            "kapanmayi reddeden exact kurulu yt-dlp",
            "diger hedef surecler kapaliyken tek engelleyici",
            "exact kurulu calistirilabilir yol/pid baseline'i",
            "closeapplications=force kullanilmaz", "fail-closed",
            "sorun yok sayilip ilerlensin", "kurulum iptal edilsin",
            "setup basari marker'i bulunmamali", "rollback",
            "dosya listesi, metadata ve sha-256", "uninstall kaydi",
            "appdata", "fixture pid'leri kontrollu kapatilir",
            "sifir surec sizintisi", "exact kurulu deno aktifken",
            "restart manager yalniz deno'yu bulmali",
            "kooperatif kapanis", "kurulum basariyla tamamlanmali",
            "exit 0", "restart gerekmemeli", "bes add-on payload'i",
            "kullanici verisi ve ayarlar korunmali",
            "ana uninstall kaydi", "openwith ve kisayollar exact korunmali",
            "suspended-deno", "ayri risk/onay karariyla",
            "zorunlu b2 kapisi degildir", "birbirine aktarilmaz",
            "normal install/upgrade"):
        assert phrase in gate, f"add-on surec kapisinda eksik: {phrase}"

    assert "aktif player, yt-dlp ve deno surecleriyle gercek upgrade" not in gate
    assert "yt-dlp ve exact kurulu deno'nun her biri ayri senaryoda" not in gate
