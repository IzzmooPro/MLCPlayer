...
def test_historical_status_cannot_claim_to_be_the_current_start():
    opening = "\n".join(read(PROJECT_HISTORY).splitlines()[:10])
    assert "tarihsel proje kaydı" in opening
    assert "Güncel başlangıç noktası bu dosya değildir" in opening
    assert "docs/CONTINUITY.md" in opening
...
def test_current_state_points_to_the_latest_evidence_and_a_known_commit():
    ...
    assert "Cryptography wheel" in text
    assert "libmpv" in text and "tekrar çalıştırma" in text
...
def test_session_start_injects_the_current_next_step_not_stale_history():
    ...
    assert "Cryptography wheel" in context
    assert "libmpv" in context and "tekrar çalıştırma" in context
